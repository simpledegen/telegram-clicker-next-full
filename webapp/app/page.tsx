...
export default function Page() {
  ...
  // 👇 Adaugă acest al doilea useEffect imediat după primul
  useEffect(() => {
    let t: any;
    function start() {
      stop();
      t = setInterval(() => refresh(), 2000); // 2 secunde
    }
    function stop() {
  ...
  async function onClick() {
    // UI optimist ca să se simtă instant
    setMine((m) => m + 1);
    setGlobal((g) => g + 1);
    ...
    try {
      ...
      if (Array.isArray(r?.top)) setTop(r.top); // ✅ update live pentru leaderboard
    } catch {
      await refresh();
    } finally {
